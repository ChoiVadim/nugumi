...
enum ToolAgentLiveBuilderError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .runtimeUnavailable:
            return "The tool-building agent isn't available in this copy of Gizmo."
        ...
        case .failed(let failure):
            ...
            case .workerFailure:
                return "Gizmo couldn't complete the tool checks."
            case .invalidCandidate:
                return "The model described a tool Gizmo can't represent. Try again or choose another model."
            case .attestationFailed:
                return "The finished tool didn't match the one Gizmo tested, so it was discarded."
            case .outputLimit:
                return "The tool produced far more output than Gizmo can check."
            case .invalidProtocol:
                ...
                return "Gizmo couldn't understand its own tool agent. This build's agent and app don't match."
            }
        }
    }
}
...
///
/// Running a candidate on a fixture is the only evidence Nugumi has that a
...
enum ToolAgentLiveBuilder {
    ...
    private static func build(
        ...
        let store = try ToolBuildStore(directoryURL: NugumiPaths.toolAgentRuns)
        ...
        let supervisor = makeSupervisor(
            ...
            makeProcess: { _ in
                let process = try JSONLProcess.launch(
                    ...
                    environment: ["TMPDIR": NugumiPaths.toolAgentRuns.path]
                )
