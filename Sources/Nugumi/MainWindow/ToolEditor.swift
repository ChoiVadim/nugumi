...
enum ToolEditorDraftVerification {
    static func fingerprint(tool: NugumiTool, script: String, brief: String) -> String {
...
struct ToolEditorPanel: View {
    ...
    @State private var draft = NugumiTool()
    ...
    /// Set only for a generated Python tool, where "did Gizmo actually run
    ...
    private var headerSubtitle: String {
        if page == .overview {
            return isNew ? "Build it with Gizmo" : "Edit it with Gizmo"
        }
    ...
    private var detailsContent: some View {
        VStack(alignment: .leading, spacing: 26) {
            ...
            case .prompt:
                editorSection(
                    "Instruction",
                    subtitle: "The exact prompt Gizmo sends to your current model."
                ) {
                    VStack(alignment: .leading, spacing: 18) {
                        promptField
                        languageToggle
    ...
    private static func subtitle(for kind: ToolKind) -> String {
        switch kind {
        case .prompt:
            return "Your prompt runs over the selected text. Gizmo adds nothing else to it."
        case .native:
    ...
    private var consentNotice: some View {
        HStack(alignment: .top, spacing: 9) {
            Image(systemName: "exclamationmark.triangle")
                .font(.system(size: 12, weight: .semibold))
                .foregroundStyle(Color(red: 1.0, green: 0.78, blue: 0.42))
            Text("A script tool runs real code with your account's access. What you set above is shown to you before each new version runs — it is not a restriction Gizmo enforces. Read the code before you allow it.")
                .font(.system(size: 11.5))
                .foregroundStyle(FlowTheme.inkSecondary)
                .fixedSize(horizontal: false, vertical: true)
        }
