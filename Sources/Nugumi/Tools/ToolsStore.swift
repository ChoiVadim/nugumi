...
/// The user's tools, one folder each under `NugumiPaths.tools`:
///
...
final class ToolsStore: ObservableObject {
    @Published private(set) var tools: [NugumiTool] = []
    var onChange: (() -> Void)?
    ...
    init(directoryURL: URL = NugumiPaths.tools, migrateLegacy: Bool = true) {
    ...
    func tool(id: UUID) -> NugumiTool? {
        tools.first { $0.id == id }
    }

    func usableTools() -> [NugumiTool] {
    ...
    func isRunnable(_ tool: NugumiTool) -> Bool {
    ...
    @discardableResult
    func save(_ tool: NugumiTool, script: String? = nil) -> NugumiTool {
    ...
    private func load() {
        ...
        tools = entries.compactMap { dir -> NugumiTool? in
            ...
            return try? decoder.decode(NugumiTool.self, from: data)
        }
    ...
    private func migrateFromDefaultsIfNeeded() {
        ...
        guard let legacy = try? JSONDecoder().decode([NugumiTool].self, from: data) else { return }
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
        for tool in legacy {
            let dir = directory(for: tool.id)
