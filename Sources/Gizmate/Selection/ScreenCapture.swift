...
enum ScreenshotTranslationError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .screenRecordingPermissionDenied:
            "Gizmate needs Screen Recording permission to capture screenshots. Open settings to enable it, then choose Quit & Reopen to apply the change."
        }
    }
...
struct AskGizmateScreenCapture {
...
extension AskGizmateScreenCapture {
    ...
    func annotated(with strokes: [[NSPoint]]) -> AskGizmateScreenCapture {
...
enum ScreenshotCapture {
    @MainActor
    static func captureActiveScreen(containing point: NSPoint = NSEvent.mouseLocation) async throws -> AskGizmateScreenCapture {
    ...
    static func captureInteractiveArea() async throws -> URL {
        ...
        // which is what registers Gizmate in System Settings. Calling CGRequestScreenCaptureAccess
        // here would stack Apple's system prompt on top of our GizmateAlertController.
        guard CGPreflightScreenCaptureAccess() else {
            throw ScreenshotTranslationError.screenRecordingPermissionDenied
        }
        ...
        return try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                let outputURL = FileManager.default.temporaryDirectory
                    .appendingPathComponent("gizmate-screenshot-\(UUID().uuidString)")
