...
final class PetController: NSObject, NSTextFieldDelegate {
    ...
    private var onTool: ((GizmateTool, String) -> Void)?
    ...
    private var currentPromptInputLayout = AskGizmatePromptInputMetrics.layout(forContentHeight: 0)
    private var currentAnswerLayout = AskGizmateAnswerBubbleMetrics.layout(forContentHeight: 0)
    ...
    init(initialMode: TranslationMode) {
        ...
        let initialPromptInputLayout = AskGizmatePromptInputMetrics.layout(forContentHeight: 0)
        ...
        let initialAnswerLayout = AskGizmateAnswerBubbleMetrics.layout(forContentHeight: 0)
        ...
        promptTextField.font = GizmateFont.pixelPrompt(size: 16)
        ...
        answerTextView.font = GizmateFont.pixelPrompt(size: Self.answerFontSize)
        ...
        continueButton.contentTintColor = .gizmateAccent
    ...
    /// Dresses the pet in the writing register Gizmate will use for the frontmost app
    ...
    private func refreshStyleBadge() {
        ...
        guard let runningApp = NSWorkspace.shared.frontmostApplication,
            ...
            return // keep the last register while Gizmate itself is frontmost
        }
    ...
    func showAnswer(_ message: String, emotion: AskGizmateEmotion?) {
        radialMenu?.close()
        radialMenu = nil
    ...
    private func configurePromptTextFieldForInput() {
        promptTextField.font = GizmateFont.pixelPrompt(size: AskGizmatePromptInputMetrics.fontSize)
    ...
    private func configureAnswerTextView(with message: String) {
        ...
        currentAnswerLayout = AskGizmateAnswerBubbleMetrics.layout(forContentHeight: contentHeight)
        let layout = currentAnswerLayout
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineBreakMode = .byWordWrapping

        let attributes: [NSAttributedString.Key: Any] = [
            .font: GizmateFont.pixelPrompt(size: Self.answerFontSize),
    ...
    private func answerContentHeight(for message: String) -> CGFloat {
        ...
        let boundingRect = (message as NSString).boundingRect(
            with: NSSize(
                width: AskGizmateAnswerBubbleMetrics.layout(forContentHeight: 0).viewportFrame.width,
                height: .greatestFiniteMagnitude
            ),
    ...
    private func setPromptPlaceholder(_ text: String) {
        ...
        promptTextField.placeholderAttributedString = NSAttributedString(
            ...
            attributes: [
                ...
                .font: promptTextField.font ?? GizmateFont.pixelPrompt(size: 16)
            ]
        )
    }
    ...
    private func refreshPromptInputLayout() {
        currentPromptInputLayout = AskGizmatePromptInputMetrics.layout(
    ...
    private func promptInputContentHeight(for text: String) -> CGFloat {
        ...
        paragraphStyle.lineBreakMode = .byWordWrapping
        let attributes: [NSAttributedString.Key: Any] = [
            .font: GizmateFont.pixelPrompt(size: AskGizmatePromptInputMetrics.fontSize),
            .paragraphStyle: paragraphStyle
        ]
        let measurementSize = NSSize(
            width: AskGizmatePromptInputMetrics.textMeasurementWidth,
            height: .greatestFiniteMagnitude
        )
        ...
        return ceil(rawHeight) + AskGizmatePromptInputMetrics.textMeasurementBottomInset
    }

    /// Give the prompt's native NSTextField keyboard focus so the blinking
    /// caret appears. The panel becomes key without activating Gizmate
