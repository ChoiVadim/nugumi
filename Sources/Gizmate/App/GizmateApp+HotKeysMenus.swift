...
extension GizmateApp {
    ...
    func setupGlobalHotKeys() {
        ...
        // the action ids 1...7.
        let askGizmateAlias = GlobalHotKey(
            definition: GlobalHotKeyDefinition(id: 100, shortcut: GlobalShortcutAction.askGizmateAlias),
            onPressed: { [weak self] in self?.startAskGizmatePrompt() }
        )
    ...
    private func makeStatusBarMenu() -> NSMenu {
        ...
        let quit = NSMenuItem(title: "Quit Gizmate", action: #selector(quit), keyEquivalent: "q")
        quit.target = self
        menu.addItem(quit)
        return menu
    }
    ...
    @objc func contactSupport() {
        ...
        let subject = "Gizmate bug or request"
        let body = """
        Hey Vadim,
    ...
    /// delivered through the Edit menu. The menu surfaces only while Gizmate is the
    /// active app. ⌘Q is deliberately bound to "Close Window" rather than Quit:
    /// Gizmate lives in the menu bar, so closing the window must not kill it — users
    /// quit via the status-bar "Quit Gizmate" item.
    func installMainMenu() {
        ...
        appMenu.addItem(withTitle: "Hide Gizmate",
