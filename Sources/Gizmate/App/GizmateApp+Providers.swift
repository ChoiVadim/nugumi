...
extension GizmateApp {
    ...
    @MainActor
    func disconnectCloudProvider(_ provider: CloudProvider) {
        ...
        let response = GizmateAlertController(
            ...
            message: isOAuth
                ? "Gizmate will forget this account. Models from \(provider.displayName) stop working until you sign in again."
    ...
    @MainActor
    func applyModelSelection(_ modelID: String, for scope: ModelUseScope) {
    ...
    func applyEnginePreset(_ engine: EngineModelPreset, force: Bool = false) {
        var applied = false
