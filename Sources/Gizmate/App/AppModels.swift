...
struct LLMModel: Equatable {
    ...
    static let all: [LLMModel] = [
        ...
        // The one curated local vision model, so Ask Gizmate works on a
    ...
    private static func makeOllamaModel(name: String) -> LLMModel {
        ...
        return LLMModel(
            ...
            // (`/api/show`); only vision models surface in Ask Gizmate.
...
/// Everyday text gets the fast/cheap tier, Ask Gizmate the vision flagship.
...
enum LaunchAtLogin {
    ...
    static func set(_ enabled: Bool) {
        do {
            ...
            NSLog("[Gizmate] Launch-at-login \(enabled ? "register" : "unregister") failed: \(error.localizedDescription)")
        }
    }
}
