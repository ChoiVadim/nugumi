...
final class UVBootstrap: ObservableObject {
    ...
    /// Where we look, in order. Gizmate's own copy wins so an install we made is
    /// used even if the user later adds a different one to their PATH.
    private static var candidatePaths: [URL] {
        [
            GizmatePaths.bin.appending(path: "uv", directoryHint: .notDirectory),
    ...
    private static func unpack(archive: URL) throws -> URL {
        ...
        let destination = GizmatePaths.bin.appending(path: "uv", directoryHint: .notDirectory)
    ...
    /// package cache inside Application Support so Gizmate never writes into the
    ...
    nonisolated static func environment() -> [String: String] {
        ...
        env["UV_CACHE_DIR"] = GizmatePaths.cache.path
        env["UV_PYTHON_INSTALL_DIR"] = GizmatePaths.pythonInstall.path
