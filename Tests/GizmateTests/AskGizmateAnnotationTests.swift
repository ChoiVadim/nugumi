...
final class AskGizmateAnnotationTests: XCTestCase {
    private func parse(_ annotationsJSON: String) -> AskGizmateResponse {
        AskGizmateResponse.parse(
    ...
    func testAbsentAnnotationsFieldYieldsEmptyArray() {
        let response = AskGizmateResponse.parse("{\"message\":\"plain\"}")
    ...
    func testRoundTripPreservesValidAnnotations() throws {
        let original = AskGizmateResponse(
            message: "look",
            annotations: [
                AskGizmateAnnotation(
                    type: .arrow,
    ...
    func testMalformedAnnotationsFieldFallsBackToEmpty() {
        let response = AskGizmateResponse.parse(
    ...
    func testExactScreenPointMapsAsymmetricPoint() {
        let point = AskGizmateCoordinateMapper.exactScreenPoint(
            normalizedX: 0.25,
            normalizedY: 0.10,
            screenFrame: frame
        )
    ...
    func testScreenRectCentersOnMappedPoint() {
        let rect = AskGizmateCoordinateMapper.screenRect(
            centerX: 0.5,
            centerY: 0.25,
            normalizedWidth: 0.1,
    ...
    func testCoordinateGuideCoversAnnotations() {
        let withImage = AskGizmatePromptBuilder.prompt(question: "where?", hasImage: true)
        XCTAssertTrue(withImage.contains("annotations"))

        let withoutImage = AskGizmatePromptBuilder.prompt(question: "where?", hasImage: false)
