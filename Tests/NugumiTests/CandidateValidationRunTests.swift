...
final class CandidateValidationRunTests: XCTestCase {
    private func uv() throws -> URL {
        let candidates = [
            URL(fileURLWithPath: NSHomeDirectory())
                .appending(path: "Library/Application Support/Nugumi/bin/uv"),
    ...
    func testNoFixturesResolvesDependenciesWithoutRunningTheScript() async throws {
        ...
        let marker = FileManager.default.temporaryDirectory
            .appending(path: "nugumi-must-not-run-\(UUID().uuidString)")
        ...
        let unresolvable = try candidate(
            source: """
            # /// script
            # dependencies = ["nugumi-package-that-does-not-exist==1.0.0"]
            # ///
            print("ok")
            """,
            fixtures: []
