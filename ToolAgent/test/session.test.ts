...
test("model action contract accepts every runnable candidate shape and rejects invalid outputs", () => {
  ...
  assert.doesNotThrow(() => parseModelAction(action({ ...python, fixtures: [] })));
  assert.doesNotThrow(() =>
    parseModelAction(
      action({
        ...python,
  ...
  assert.throws(
    () =>
      parseModelAction(
        action({ ...python, fixtures: [{ input: "ok", expectedOutput: "OK", extra: 1 }] }),
      ),
  ...
  assert.throws(
    () => parseModelAction(action({ ...python, output: "files", outputDirectory: "" })),
    /invalid tool arguments/,
  );
  assert.throws(
...
test("live Pi fix session gives the model the current tool and exact failure", async () => {
  ...
  const result = await drive(child, (message) => {
    ...
    assert.equal(initialRequest["failure"], "Expected HELLO but received hello.");
...
test("live bridge serializes five tools and prior structured validation detail", async () => {
  ...
  const result = await drive(child, (message) => {
    ...
    if (kind === "modelRequest") {
      ...
      assert.match(system, /UNSUPPORTED is a last resort and never a keyword filter/);
      assert.match(
        system,
      ...
      assert.match(
        system,
        /repair from its diagnostics instead of giving up/,
      );
      ...
      assert.match(
        system,
        /"nativeAction":"openURL","target":"\{input\}"/,
      );
      assert.match(
        system,
      ...
      assert.match(system, /One fixture with expectedOutput, when the same input always produces the same\s+output/);
      assert.match(system, /One fixture without expectedOutput, when running it proves something real/);
      assert.match(system, /No fixtures at all, when running the script would do something to the user's\s+data/);
      assert.match(system, /Never invent a fixture whose only purpose is to pass/);
      assert.match(
        system,
        /write results into the current working directory with\s+relative paths/,
      );
      assert.match(system, /Printing success\s+without writing a file fails validation/);
      ...
      assert.doesNotMatch(system, /yt-dlp|nugumi\.invalid|standard library only|Downloaded video/);
  ...
  const result = await drive(child, (message) => {
    ...
    if (message["type"] === "modelRequest") {
      ...
      if (modelTurn === 1) {
        ...
        assert.doesNotMatch(system, /For Create only|Never call ask_user for edit or fix/);
      }
      if (modelTurn === 2) {
        const context = object(JSON.parse(text(payload["user"])));
      ...
      const actions = [
        {
          ...
          action: "toolCall",
          name: "ask_user",
          arguments: { question: "Which app should receive the selected text?" },
        },
        {
          version: 1,
          action: "toolCall",
    ...
    if (name === "ask_user") {
      ...
      send(child, "toolResponse", {
        ...
        result: {
          name,
          payload: { candidateID: firstCandidateID, fingerprint: firstFingerprint },
        },
      });
    }
  });
...
test("live Pi session accepts and attests a native Notes candidate", async () => {
  ...
  const result = await drive(child, (message) => {
    ...
    if (message["type"] === "modelRequest") {
      ...
      const actions = [
        ...
        },
        {
          ...
          arguments: { candidateID: firstCandidateID, fingerprint: firstFingerprint },
        },
    ...
    if (name === "read_build_context") {
      ...
      send(child, "toolResponse", {
        callID,
        result: { name, payload: { candidateID: firstCandidateID, fingerprint: firstFingerprint } },
      });
    } else if (name === "run_validation") {
      send(child, "toolResponse", {
        callID,
